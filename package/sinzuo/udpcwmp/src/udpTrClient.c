...
#define DEVICE_TYPE_SHELL "cat /etc/device_info |grep DEVICE_PRODUCT|awk -F \"\'\" '{print $2}'"
#define SOFT_VERSION_SHELL "cat /etc/device_info |grep DEVICE_REVISION|awk -F \"\'\" '{print $2}'"
...
int GetBoolByEtype(json_object *jobj, const char *sname)
{
  json_object *pval = NULL;
  enum json_type type;
  pval = json_object_object_get(jobj, sname);
...
int getDeviceTypeAndVersion()
{
  char buff[128];
  char value[128];
  read_memory(SOFT_VERSION_SHELL,buff,128  );
  util_strip_traling_spaces(buff);
  strcpy(deviceVer,buff);
  printf("jiangyibo 223%s\n",buff);
  read_memory(DEVICE_TYPE_SHELL,buff,128  );  
  util_strip_traling_spaces(buff);
  printf("jiangyibo 22 %s\n",buff);
  strcpy(deviceType, buff);
}
