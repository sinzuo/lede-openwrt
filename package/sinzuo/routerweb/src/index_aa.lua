...
--        nixio.stdclose( "oe" )
        ...
        status = luci.sys.call( "/usr/sbin/webfactory.sh" )
...
function Api_wip_set(args)
    ...
    local wifimode = uci:get( "network", "wan", "proto"  ) or "wifi"
    if wifimode == "wifi" then
...
function Api_wip_get(args)
    ...
    local ret = {
        wantype = uci:get(fname, section, "_wantype")
    }

    if not ret.wantype then
        local ifname = uci:get( fname, section, "ifname" )
        local proto = uci:get( fname, section, "proto" )
        ret.wantype = ifname and proto or "wifi"
    end
    local real = {}
    if ret.wantype == "wifi" then
    ...
    local encr = uci:get( "wireless", "relay", "encryption" ) or "none"
    ret.wifi = {
        disabled = tonumber( uci:get( "wireless", "relay", "disabled" ) or "0" ),
        ssid = uci:get( "wireless", "relay", "ssid" ) or "",
        key  = uci:get( "wireless", "relay", "key"  ) or "",
   ...
   local curHttpRegister = util.exec("/bin/httpreg") 
    ret.real = {
